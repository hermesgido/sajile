...
class PostController extends Controller
{
    ...
    public function events_post_list()
    {
        ...
        return view('admin.posts.events_list', compact('pageTitle', 'posts', 'emptyMessage'));
    }
    ...
    protected function postData($type = null, $scope = null)
    {
        ...
        if ($request->search) {
            ...
            $posts  = $posts->where('type' , $type)->where(function ($post) use ($search) {
                $post->where('title', 'like', "%$search%")
                    ->orWhere('content', 'like', "%$search%");
            });
        }        
        return $posts->with('user','category','apply_job','events')->where('type' , $type)->orderBy('id', 'desc')->paginate(getPaginate());
    }
